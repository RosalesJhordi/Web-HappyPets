<div class="relative ">
    ...
    <div class="flex w-full gap-5 py-5">
        @livewire('admin.secciones.ventas-count')
        <button wire:click='$toggle("seccionVenta")' type="button"
            ...
            <span>
                <h2 class="text-sm font-semibold">Nueva Venta</h2>
            </span>
        </button>
    </div>
    @if ($seccionVenta)
        <button wire:click='$toggle("seccionVenta")' type="button"
            class="flex items-center justify-between w-auto gap-2 px-3 py-1 text-xl text-blue-600 bg-blue-200 rounded-md">
            <i class="text-sm fa-solid fa-x"></i>
            cerrar
        </button>
        <div class="flex flex-col justify-between w-full px-1 md:flex-row red-500 xl:max-w-8xl">

            <div class="hidden md:flex overflow-y-auto py-2 sm:py-10 flex-col w-full h-[90vh] md:w-[15%]"
            ...
            <div class="px-4 overflow-y-auto py-2 mx-auto sm:px-6 sm:py-10 w-full md:w-[85%] lg:px-8"
                style="scrollbar-width: none; -ms-overflow-style: none;">
                ...
                <div wire:loading.remove class="w-full">
                    @if (!empty($datosProducto))
                        <div class="grid grid-cols-2 mt-6 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-5">
                            @foreach ($datosProducto as $data => $producto)
                                <div class="flex flex-col border rounded-md "
                                    ...
                                    <div class="z-30 px-2 py-2">
                                        ...
                                        <dialog wire:key="producto-{{ $loop->index }}-{{ $producto['id'] }}"
                                            ...
                                            @livewire('admin.carrito', [$producto['nm_producto'], $producto['imagen'], $producto['id'], $producto['descuento'], $producto['precio'], $producto['descripcion'], $producto['colores'], $producto['stock']], key('carrito-' . $loop->index . '-' . $producto['id']))
                                        </dialog>
                                    </div>
                                </div>
                            @endforeach
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($datosTabla as $venta)
                        <tr>
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-10 h-10">
                                        <img class="w-10 h-10 rounded-md"
                                            src="https://api.happypetshco.com/ServidorProductos/{{ $venta['producto']['imagen'] }}"
                                            alt="{{ $venta['producto']['nm_producto'] }}">
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $venta['producto']['nm_producto'] }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                S/. {{ number_format($venta['producto']['precio'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
